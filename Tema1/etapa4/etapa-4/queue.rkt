...
(define (rotate left right Acc)
  (cond
    ((and (stream-empty? left) (null? right)) Acc )
    ((stream-empty? left) (rotate left (cdr right) (stream-cons (car right) Acc)))
    ((null? right) (stream-cons (stream-first left) (rotate (stream-rest left) right Acc)))
    (else (stream-cons (stream-first left) (rotate (stream-rest left) (cdr right) (stream-cons (car right) Acc))))
    ))
...
(define (enqueue x q)
  (cond
    ((> (queue-size-l q) (queue-size-r q)) (make-queue (queue-left q) (cons x (queue-right q)) (queue-size-l q) (+ (queue-size-r q) 1)))
    (else (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (+ 1 (queue-size-r q) (queue-size-l q)) 0))))
...
(define (dequeue q)
  (cond
    ((> (queue-size-l q) (queue-size-r q)) (make-queue (stream-rest (queue-left q)) (queue-right q) (- (queue-size-l q) 1) (queue-size-r q)))
    (else (make-queue (stream-rest (rotate (queue-left q) (queue-right q) empty-stream)) '() (- (+ (queue-size-r q) (queue-size-l q)) 1) 0))))
